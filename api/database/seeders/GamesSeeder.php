...
class GamesSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        while ($d->lte($now)) {
            $i++;
            if (
                ($filteredPlayers === null) || ($nextCreatedAt === null) ||
                ($d->gte($nextCreatedAt))
            ) {
                ...
                $nextCreatedAt = $nextCreatedAtPlayer ? $nextCreatedAtPlayer->created_at : new Carbon('9999-12-31');
            ...
            if (rand(1, $this->ratioStandaloneToMatch) === 1) {
                $matchData = $this->newMatch($filteredPlayers, $userIDPlayer1, $userIDPlayer2, $d);

                $matchData['id'] = DB::table('matches')->insertGetId($matchData);

                $playersMarks = [0, 0];
                $playersPoints = [0, 0];

                while ($playersMarks[0] < 4 && $playersMarks[1] < 4) {
                    $newGame = $this->newGame($filteredPlayers, $matchData, $userIDPlayer1, $userIDPlayer2, $d);
                    $games[] = $newGame;
                ...
                $this->updateMatchWinner($matchData, $playersMarks[0], $playersMarks[1], $playersPoints[0], $playersPoints[1], $d);
                $idToUpdate = $matchData['id'];
                unset($matchData['id']);
            } else {
                $newGame = $this->newGame($filteredPlayers, null, $userIDPlayer1, $userIDPlayer2, $d);
                $games[] = $newGame;
            }
